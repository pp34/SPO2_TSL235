...
const uint8_t reverse_data[] = { 0x0, 0x8, 0x4, 0xC, 0x2, 0xA, 0x6, 0xE, 0x1, 0x9, 0x5, 0xD, 0x3, 0xB, 0x7, 0xF };
...
void LCD_clearCS(void) {
	P6OUT &= ~BIT6;
}
void LCD_setCS(void) {
	P6OUT |= BIT6;
}
void LCD_disableDisplay(void) {
	P1OUT &= ~BIT6;
	P6OUT &= ~BIT5;
}
void LCD_enableDisplay(void) {
	P1OUT |= BIT6;
	P6OUT |= BIT5;
}
